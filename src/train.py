...
if not CFG_FILE.exists():
    raise FileNotFoundError("Configuration file not found – create it under config/exp.yaml before running.")
...
if not DIT_REPO.exists():
    ...
    # Rename .git to avoid accidental nested-repo operations
    (DIT_REPO/".git").rename(DIT_REPO/"_git")
...
class FFTDiT_S(nn.Module):
    ...
    def forward(self, x:torch.Tensor, t:torch.Tensor):
        ...
        x = self.ln_out(x)
        x = self.proj(x).view(B, 3, 4, 4, -1)
        x = torch.nn.functional.pixel_shuffle(x.flatten(0,1), 2)
        return x
